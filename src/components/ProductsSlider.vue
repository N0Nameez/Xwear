...
import ProductCard from './WearCardElement.vue';
...
const chunks = computed(() => {
    ...
    totalSlides.value = result.length;
    return result;
});

function goToSlide(index) {
    if (index < 0) {
        index = totalSlides.value - 1;
...
<template>
    <div class="products-slider">
        <div class="slider-header">
            <h2 class="slider-title">Обувь</h2>
        ...
        <div v-else-if="products.length === 0" class="loading">Нет товаров для отображения</div>
