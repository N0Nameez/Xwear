...
const formattedPrice = computed(() => {
    return props.product.price ? `${props.product.price} ₽` : 'Цена не указана';
});

const productTitle = computed(() => {
    return props.product.title || `${props.product.brand || ''} ${props.product.model || ''}`.trim();
});
...
<template>
    <div class="wear-card" @click="navigateToProduct">
        ...
        <img :src="product.imageUrl" :alt="productTitle" class="product-image">
        <p class="good-name">{{ productTitle }}</p>
        <p class="good-price">{{ formattedPrice }}</p>
    </div>
</template>
